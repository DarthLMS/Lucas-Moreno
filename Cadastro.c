...
void preencheVetor (CADASTRO cadastro[]) {
    for (int i = 0; i < numCad; i++) {
        ...
        printf("Escreva a cidade da %d° pessoa(máximo 30 caracteres): ", i+1);
         ...
         do
        {
            ...
            if(cadastro[i].estadoCivil < '1' || cadastro[i].estadoCivil > '4')
                printf("--Opção inválida, por favor digite conforME o mostrado--\n");
        }
...
void Idade(CADASTRO cadastro[]){
     int maior, posmIdade;

     maior = cadastro[0].idade;
     for(int i = 1; i < numCad; ++i)
...
void Salario(CADASTRO cadastro[]){
    for(int i = 0; i < numCad; ++i)
        if(cadastro[i].salario > 1000)
            printf("\nSalario acima de 1000:\nCadastro: %d\nNome: %s\n\n",i+1,cadastro[i].nome);
}



int RG(CADASTRO cadastro[]){
